...
class MixinPartner(models.AbstractModel):
    ...
    * Computed boolean fields that expose the effective required/readonly
      ...
      and the current record state — so views can use them in ``invisible``,
      ``readonly`` and ``required`` expressions.
    * Optional view injection into form, list, and search views controlled
