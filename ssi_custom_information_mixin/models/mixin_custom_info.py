...
class MixinCustomInfo(models.AbstractModel):
    ...
    def onchange(self, values, field_names, fields_spec):
        ...
        if x2many_field in fields_spec:
            comodel = self._fields[x2many_field].comodel_name
            sub_spec = fields_spec[x2many_field].setdefault("fields", {})
            for subfield in self.env[comodel]._fields:
                sub_spec.setdefault(subfield, {})
    ...
    def _evaluate_custom_info_use_python(self, template):
        ...
        try:
            safe_eval(template.python_code, localdict, mode="exec")
