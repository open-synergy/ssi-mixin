...
class MixinMultipleApproval(models.AbstractModel):
    ...
    def _search_validated(self, operator, value):
        ...
        pos = self.search(
            [(self._approval_state_field, "in", self._approval_state_from)]
                ...
                if rec.need_validation:
                    ...
                    if not self._get_approvals_approved(reviews):
                        raise ValidationError(
                            _(
                                "This action needs to be approved for at least "
                                "one record. \nPlease request an approval."
                            )
                        )
                if rec.approval_ids and not rec.approved:
                    raise ValidationError(
                        _(
                            "An approval process is still open for at least "
                            "one record."
                        )
                    )
            if (
