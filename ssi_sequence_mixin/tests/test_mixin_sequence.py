...
class TestMixinSequence(YamlTransactionCase):
    def test_mixin_sequence(self):
        ...
        # model "test.sequence" (module "test_ssi_sequence_mixin"). Guard
        ...
        if "test.sequence" not in self.env:
            self.skipTest(
                "Model 'test.sequence' is not available - module "
                "'test_ssi_sequence_mixin' is not installed."
            )
